...
// RM电机角度补偿 0x201-0x213对应m1-m13,直接加载期望零点时反馈角度
// C6x0 1-8对应m1-m8
// 6020 1-7对应m5-m13
...
public:
    ...
    */
    ...
    // static void sendRMMotor(TPCANHandle CANx, DWORD ID, WORD M1, WORD M2, WORD M3, WORD M4, bool is_open);
    // static void sendDMMotor(TPCANHandle CANx,int motor_id,float _pos, float _vel,float _KP, float _KD, float _torq, bool is_open);
    // /*通道 发布者 消息帧id 电机类型
    // 获取RM电机直接打开即可 每个通道独立线程
    // */
    // void readRMMotor(TPCANHandle CANx, rclcpp::Publisher<hongying_ctrl_msg::msg::MotorFeedback>::SharedPtr publisher, 
    // std::string frame_id, MotorType motor_type, CompensateAngle compensate_angle=CompensateAngle());
    //     //达妙给正逆时针旋转 逆时针选装位置数值增加
    // void readDMMotor(TPCANHandle CANx, rclcpp::Publisher<hongying_ctrl_msg::msg::MotorFeedback>::SharedPtr publisher, 
    // std::string frame_id, MotorType motor_type, CompensateAngle compensate_angle=CompensateAngle());
    // /*多个can使用一个线程
    //  */
    // std::vector<std::function<void()>> rmmotor_rvs;
    // void addRMMotor(TPCANHandle CANx, rclcpp::Publisher<hongying_ctrl_msg::msg::MotorFeedback>::SharedPtr publisher, 
    // std::string frame_id, MotorType motor_type, CompensateAngle compensate_angle=CompensateAngle());
    // std::thread rmmotro_rv_thread;
    // std::atomic<bool> rmmotro_rv_thread_state{true};
    // void runRMMotorRv();

    // void addDMMotor(TPCANHandle CANx, rclcpp::Publisher<hongying_ctrl_msg::msg::MotorFeedback>::SharedPtr publisher, 
    // std::string frame_id, MotorType motor_type, CompensateAngle compensate_angle=CompensateAngle());

    // //组合电机3508仍然是ID1-8，达妙与RM电机ID不冲突，不过数据不同
    // void addMotor(TPCANHandle CANx, rclcpp::Publisher<hongying_ctrl_msg::msg::MotorFeedback>::SharedPtr publisher, 
    // std::string frame_id, MotorType motor_type, CompensateAngle compensate_angle=CompensateAngle());

    // void enableDMotor(TPCANHandle CANx, uint8_t id, bool enable);
};
